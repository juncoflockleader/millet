...
The editor stores a full local `BoardLayoutJson` draft in browser `localStorage` and exports/imports the same JSON shape. Designers can start from the ruleset default, the Hearthstone-like `2P Duel` board, or the Sanguosha-like `8-Seat Identity` board template before editing individual regions. Token controls are still optimized for the Hearthstone-like 1v1 board, while the region editor supports selecting authored regions, editing label/kind/owner/widget/visibility/drop/accepts/targetability/data-source fields, editing the selected widget's id/kind/component/config JSON, and changing `x`, `y`, `width`, and `height` through form fields or direct canvas drag/resize. Region editing also supports an 8px snap grid, duplicate, horizontal flip, vertical flip, fill-to-board, and delete actions so designers can block out symmetric or large table surfaces without hand-calculating every coordinate. The first runtime data-source binding is `dataSource.zoneType`, which lets core 1v1 widgets resolve player-owned hand, battlefield, weapon, and deck zones from the layout instead of relying only on conventional zone ids.
...
The `Playtest` panel is the first integrated debug surface in Millet Studio. It shows whether the active project has browser-local layout, card catalog, presentation, or asset drafts, then loads authored playtest scripts from the ruleset's `game-definition.ui.defaultPlaytestScript`. In `Ember Duel`, `packages/rulesets/sample-duel/ui/ember-duel-playtests.json` declares `Firebolt Smoke`: create a demo match, cast P1 `Firebolt` into P2 through `/matches/:id/commands`, fetch admin state/replay, assert Player 2 health, update the board to the generated match, and summarize event count, damage events, sequence, and recent replay lines. The run result also includes a state diff from match creation to final script state and a replay inspector for selecting events and reading their raw projected payload JSON. This connects authoring drafts to a repeatable runtime check without making client-side rules the source of truth; unpromoted browser drafts affect Studio rendering, while engine rules still come from the served ruleset content.
