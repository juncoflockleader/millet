...
- Hero Studio smoke confirmed the `Presentation` panel opens a structured hero editor for `heroes:p1`, edits hero name, ability action, mana cost, and art fit into `ember-duel.presentation.sample-duel.v1`, updates the JSON field and preview card, then starts a match with the runtime hero card showing the edited title/action/style while the play area remains inside the viewport.
- Hero ability display smoke confirmed Hero Studio validates required ability fields, edits ability badge slot/label/priority into `ember-duel.presentation.sample-duel.v1`, updates the preview badge, and starts a match with the runtime hero card using the drafted badge layout.
...
- Card property layout smoke confirmed the `Cards` panel edits `Training Axe` display layout, changes a badge slot through registry-backed controls, add/removes a display badge, writes the result to `ember-duel.cards.sample-duel.v1`, and updates preview badge classes with no page errors.
...
- Card behavior sync smoke confirmed the `Cards` panel loads generated behavior summaries, compares `Firebolt` presentation text against generated behavior text, shows selector/effect UX hint chips, applies generated text into `ember-duel.presentation.sample-duel.v1`, updates the card preview, and handles the `sample-identity` no-presentation case without page errors.
- Card frame/art smoke confirmed the `Cards` panel edits `Firebolt` presentation frame path, art X/Y crop, and art height into `ember-duel.presentation.sample-duel.v1`, and the preview card consumes the generated CSS frame/art variables with no page errors.
- Card catalog promotion smoke confirmed the `Cards` panel shows dirty-state, target path, catalog change counts, and selected-template field diffs, posts reviewed catalog drafts to `/authoring/cards/promote`, clears `ember-duel.cards.sample-duel.v1` after success, and updates status without page errors.
