...
- Board layout widget inspector smoke confirmed the `Layout` panel shows the selected region's widget inspector, edits widget component/config into local `BoardLayoutJson`, warns on invalid config JSON, creates a missing widget for a custom region, and keeps the play area fitted inside the viewport.
- Board layout template smoke confirmed the `Layout` panel applies the Sanguosha-like `8-Seat Identity` template into the local `BoardLayoutJson` draft, updates logical size to `1280 x 720`, renders authored guide boxes, preserves viewport scaling, and can switch back to the `2P Duel` template with no page errors.
...
- Hero Studio smoke confirmed the `Presentation` panel opens a structured hero editor for `heroes:p1`, edits hero name, ability action, mana cost, and art fit into `millet.studio.presentation.sample-duel.v1`, updates the JSON field and preview card, then starts a match with the runtime hero card showing the edited title/action/style while the play area remains inside the viewport.
- Hero ability display smoke confirmed Hero Studio validates required ability fields, edits ability badge slot/label/priority into `millet.studio.presentation.sample-duel.v1`, updates the preview badge, and starts a match with the runtime hero card using the drafted badge layout.
...
- Asset authoring smoke confirmed the `Assets` panel edits a selected manifest entry JSON locally, writes `millet.studio.assets.sample-duel.v1`, rebuilds displayed prompt/usage metadata, resets back to authored manifest source, and rejects attempts to change `assetId`.
...
- Asset promotion tests confirmed `/authoring/assets/promote` writes imported image drafts to `assets/imported/<rulesetId>/`, recalculates the SHA-256 hash, updates the selected ruleset manifest entry with durable `publicPath`/`sourceUri`, and rejects non-data-URL promotion requests.
...
- Card template studio smoke confirmed the `Cards` panel opens, reads 6 `sample-duel` templates, filters by object type, previews `Firebolt` and `Training Axe`, applies a local `training_axe` JSON draft to `millet.studio.cards.sample-duel.v1`, resets back to ruleset source, and emits no page errors.
...
- Card property layout smoke confirmed the `Cards` panel edits `Training Axe` display layout, changes a badge slot through registry-backed controls, add/removes a display badge, writes the result to `millet.studio.cards.sample-duel.v1`, and updates preview badge classes with no page errors.
- Equipment Studio smoke confirmed the `Cards` panel edits `Training Axe` slot/replacement metadata, attack/durability stats, granted action label, and icon path into card/presentation local drafts, updates preview badges/action text, then starts a match with the equipped weapon card showing the drafted action while the play area remains inside the viewport.
- Equipment validation smoke confirmed the `Cards` panel shows Equipment Studio validation, warns on unsupported granted behavior ids, blocks catalog promotion on invalid equipment slot metadata, and clears the promotion block after restoring a valid weapon slot draft.
- Minion Studio smoke confirmed the `Cards` panel edits `Loot Minion` kind/token metadata, attack/health stats, modifier badge text, death trigger behavior, action label, and icon path into card/presentation local drafts, updates preview badges/action text, then starts a match with the board minion showing the drafted modifier badge and action while the play area remains inside the viewport.
- Minion validation smoke confirmed the `Cards` panel shows Minion Studio validation, warns on unknown death triggers, blocks catalog promotion on invalid token variant metadata, keeps metadata modifier badges synchronized, and clears the promotion block after restoring a valid token variant.
- Card behavior sync smoke confirmed the `Cards` panel loads generated behavior summaries, compares `Firebolt` presentation text against generated behavior text, shows selector/effect UX hint chips, applies generated text into `millet.studio.presentation.sample-duel.v1`, updates the card preview, and handles the `sample-identity` no-presentation case without page errors.
- Card frame/art smoke confirmed the `Cards` panel edits `Firebolt` presentation frame path, art X/Y crop, and art height into `millet.studio.presentation.sample-duel.v1`, and the preview card consumes the generated CSS frame/art variables with no page errors.
- Card catalog promotion smoke confirmed the `Cards` panel shows dirty-state, target path, catalog change counts, and selected-template field diffs, posts reviewed catalog drafts to `/authoring/cards/promote`, clears `millet.studio.cards.sample-duel.v1` after success, and updates status without page errors.
- Identity property layout smoke confirmed `judgment_spade_7` exposes `Suit And Point Corner`, `Suit`, and `Point` registry options, derives suit metadata from tags, and stacks suit/rank badges in the shared `suit-point` slot without visible overlap.
...
- The next playtest-debug slice added `UiPlaytestScriptJson`, ruleset playtest script refs, validation for missing/duplicate/unknown-behavior script content, an authored Ember Duel `Firebolt Smoke` script, and a Millet Studio `Playtest` panel that lists and runs project-authored scripts through match APIs with replay/event summaries, compact state diffs, event payload inspection, and preview-project routing.
- The next dogfood-game slice added `sample-rune-duel`, a second playable 1v1 Studio project with its own content, layout, presentation, preview fixture, playtest script, server ruleset registry entry, and presentation-driven runtime card actions.
- The next Rune Duel gap-fill slice added board layout `dataSource.zoneType`, runtime zone lookup through authored layout metadata, validation for unknown zone data sources, and behavior UX `visualEffect` hints for data-driven VFX selection.
- The next Basic Trio slice researched HearthstoneJSON build `6024`, documented early Basic set coverage/gaps for Mage, Warrior, Priest, and neutral cards, added and gap-filled `sample-basic-trio`, registered it as a playable Studio project, added tests/playtests for class cards, armor absorption, object damage/healing, matching-player/object area effects, matching-object freeze, deterministic random targeting, dynamic random-hit counts, hidden-info-safe random hand copy, object freeze/readiness cleanup, hero-freeze resource gating, minion combat damage plus minion/hero freeze, weapon-plus-hero-attack stacking, charge-like attack expiry, object transform, control change, attack-threshold/damaged selectors, dynamic stat expressions, numeric sum expressions, matching-object stat-sum/stat-cleanup effects, event-object trigger targeting, summon-trigger ready grants, continuous flat stat auras, targeted Battlecry minions, heal-triggered draw, Warrior all-minion/random-minion/damaged-minion/reaper/focus/ready-buff/storm-runner/drummer/banner-aura spells, Priest buff/bloom/glimpse spells, Mage stasis/column/glacier/spell-power spells, token summon, Taunt routing, hand-to-weapon play, ready-on-play minions, and generic hand-to-board minion play, and tightened compact card rendering so 2P hands stay within their viewport lane.
- The next Basic Trio UI gap-fill slice added presentation-catalog `actions`, runtime multi-action card buttons, and presentation-authored object target zone filters, then verified hero/minion variants on `Ember Lance` and `Frost Pin` through headless browser smoke.
...
- The next hero-studio slice added structured hero presentation controls, a live hero-card preview, and runtime hero art/frame/crop consumption through presentation style variables.
- The next hero-studio-hardening slice added required ability validation, behavior availability warnings, and registry-backed hero ability display badge editing in the `Presentation` panel.
...
- The next board-layout-editor slice added document-level layout fields, region presets with widget creation, editable region ids, overflow/visibility/schema-aligned field controls, local layout diagnostics, grid guides, and export preservation for custom token groups.
- The next board-layout-widget-inspector slice added selected-region widget editing for widget ids, schema kinds, renderer component names, config JSON, missing-widget creation, and widget-focused diagnostics.
...
- The next asset-authoring slice added local asset manifest entry JSON editing, draft apply/copy/reset, manifest field validation, and active usage-index rebuilds in the Ember Duel `Assets` panel.
- The next asset-import slice added browser-local image import for selected asset entries, including hash/dimension/media-type extraction and data URL previews for draft manifest entries.
...
- The next card-property-layout slice added registry-backed property display editing in the `Cards` panel, including badge add/remove, slot/icon/source/label/priority fields, and stacked same-slot badge rendering for Sanguosha-like suit/rank cards.
- The next equipment-studio slice added structured controls for equipment slot, replacement mode, attack/durability stats, frame/icon assets, granted action behavior, target mode, selector, and rules text in the `Cards` panel.
- The next equipment-studio-hardening slice added slot/replacement/stat/action validation, promotion-time equipment catalog guards, and promotion review warnings/errors for equipment templates.
- The next minion-studio slice added structured controls for minion kind, token variant, attack/health stats, frame/icon assets, attack behavior, target mode, selector, death trigger behavior, rules text, and metadata-backed modifier badge display in the `Cards` panel.
- The next minion-studio-hardening slice added kind/token/trigger/modifier validation, promotion-time minion catalog guards, and promotion review warnings/errors for minion templates.
- The next behavior-sync slice added generated behavior summaries at `/content/rulesets/:rulesetId/behavior-summaries.json` and surfaced canonical behavior text, UX hint chips, and behavior template issues in the `Cards` panel.
- The next card-frame slice added presentation-backed frame/art crop controls to the `Cards` panel and taught the runtime card renderer to consume art, frame, fit, position, and per-card art-height presentation fields.
- The next card-catalog-promotion slice added `/authoring/cards/promote`, server-side catalog shape/duplicate-id guards, Cards-panel promotion review diffs, and local draft clearing after successful promotion.
